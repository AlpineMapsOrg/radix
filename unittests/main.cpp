...
TEST_CASE("radix/main: check that asserts are enabled")
{
    CHECK(asserts_are_enabled);
}

TEST_CASE("radix/main: check that NaNs are enabled (-ffast-math removes support, -fno-finite-math-only puts it back in)")
{
